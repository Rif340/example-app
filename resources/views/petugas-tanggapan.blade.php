        ...
        <div style="flex: 1;">
            ...
            <p>Tanggal Pengaduan: {{$pengaduan->tgl_pengaduan}} Nik :{{$pengaduan->nik}}</p>
                ...
                <form method="post" enctype="multipart/form-data">
                    @method('post')
                    @csrf
                    <label for="select" style="margin-top: 2rem; ">Pilih status</label><br>
                    <select id="select" name="status"  required>
                        <option >proses</option>
                        <option >selesai</option>
                    </select><br><br>
                    <label for="pesan">Tanggapi Pengaduan :</label>
                    <textarea id="pesan"  style="width: 85%;" name="tanggapan" oninput="autoExpand(this)" required></textarea>
                    
                    <button type="submit" style="background-color: #6610f2; color: #ffffff; border-radius: 6px;">Tanggapi</button>
                </form>
        </div>
